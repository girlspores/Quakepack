...
execute if data storage cmd:global {gameState_apex:1} run tellraw @s {"text":"This game is already in progress. Please choose another map!","color":"red"}
execute if data storage cmd:global {gameState_apex:1} run return fail
# fl
forceload add 50 37
forceload add -25 -15
...
execute unless entity @p[team=apex] run function cmd:g/color/apply/apex
...
#start timer
execute if entity @a[team=apex,tag=p1] unless entity @a[team=apex,tag=p2] run schedule function cmd:g/map/apex/game/start 5t
#player index
